...
bool SMGetContentServiceInfo::processPacket(const localInfo& local)
{
	...
	*outBound << i8unk4 << possibleClasses[12]; //How do I select all from the array? also an extended int8 send for 32bytes
	*outBound << i32unk3;
	...
	*outBound << i8unk5;
	*outBound << i32unk4;
	*outBound << i8unk6 << AccessibleRegionGroupKey[75]; // Send each number in 4 bytes, not sure how to implement this?
